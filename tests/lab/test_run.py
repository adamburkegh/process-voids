...
from lab.run import (
    run, main, ALL_METRICS, GROUPS, _resolve_metrics, ProcessMetric,
    NODE_ROW_COLUMNS, PER_NODE_METRIC_KEYS, CLASSICAL_METRIC_KEYS,
...
class FakePipelineMixin:
    """See tests/lab/test_exp_disco_degrade.py's own copy - same fake
    pipeline, duplicated rather than imported since lab.run doesn't
    depend on lab.exp_disco_degrade (see lab.run's own module docstring
    on why its metric roster is a lift, not a shared import)."""
    ...
    def patch_pipeline(self, skip_probs, vm_table, timed_out_count=0, timed_out_weight=0.0,
        ...
        # there and then read each other's, so each gets an empty cache dir.
        cache_dir = tempfile.TemporaryDirectory()
        self.addCleanup(cache_dir.cleanup)
        self._patch('lab.discovery.TREE_CACHE_DIR', new=Path(cache_dir.name))
        self._patch('lab.run.pm4py.read_xes',
                    return_value=read_xes_return if read_xes_return is not None else _fake_log())
        self._patch('lab.run.build_id_net', return_value=('NET', 'IM', 'FM', {}, set(), []))
        self._patch('process_voids.metric_context.pvoid.skipprob', return_value=FakeDv(skip_probs))
        ...
        self._patch('process_voids.metric_context.voidmass_table_pn',
        ...
        self._patch('lab.run.voidsalign', return_value=voidsalign_value)
...
class MetricsSelectionTest(FakePipelineMixin, unittest.TestCase):
    ...
    def test_resolve_metrics_by_bare_id(self):
        selected = _resolve_metrics(['skipprob'])
        self.assertEqual([m.id for m in selected], ['skipprob'])

    def test_resolve_metrics_by_group_name(self):
        selected = _resolve_metrics(['classical'])
        self.assertEqual({m.id for m in selected}, set(GROUPS['classical']))

    def test_resolve_metrics_none_selects_everything(self):
        self.assertEqual(_resolve_metrics(None), ALL_METRICS)

    def test_unknown_metrics_name_raises(self):
        with self.assertRaises(ValueError):
            _resolve_metrics(['not_a_real_metric_or_group'])

    def test_resolve_metrics_accepts_a_list_of_processmetric_objects_directly(self):
        """
        A caller that already built its own inclusive/exclusive roster
        (eg lab.exp_disco_degrade's --exclude-metric complement, or its
        own default of the full ALL_METRICS list) passes ProcessMetric
    ...
    def test_selecting_one_metric_leaves_others_absent_or_null(self):
        ...
        df, node_df, timings_df = run(
            ['fake_log.xes'], combos=self.combos, degradations=self.degradations, levels=[0.5],
            metrics=['skipprob'], out_csv=str(Path(tempfile.mkdtemp()) / 'out.csv'))
        # root df has no fixed schema - an unselected metric's column is
        # absent entirely, not present-with-None.
        row = df.iloc[0]
        ...
        self.assertTrue(node_df['salign_coverage'].isna().all())

    def test_only_needed_stages_are_timed_when_metrics_selected(self):
        self.patch_pipeline({self.tree: 0.1}, self.vm_table)
        df, node_df, timings_df = run(
            ['fake_log.xes'], combos=self.combos, degradations=self.degradations, levels=[0.5],
            metrics=['skipprob'], out_csv=str(Path(tempfile.mkdtemp()) / 'out.csv'))
        stages = set(timings_df['metric_or_stage'])
        self.assertIn('dv', stages)
        self.assertNotIn('classical', stages)
...
class MetricsExclusionTest(FakePipelineMixin, unittest.TestCase):
    ...
    _null_metric_values, which is derived from the SAME metrics list
...
class NoDegradationCellShapeTest(FakePipelineMixin, unittest.TestCase):
    """degradations={} (or not given) - one row per (log, combo), no
    dim/level loop - the exp_premodel-shaped case."""

    def test_one_row_per_log_combo_with_null_degradation_fields(self):
        tree = _single_activity_tree()
        combos = {'fake': DiscoveryCombo('fake', lambda log: DiscoveryResult(tree))}
        vm_table = {tree: _fake_classical_row()}
        self.patch_pipeline({tree: 0.1}, vm_table)

        df, node_df, timings_df = run(
            ['fake_log.xes'], combos=combos, degradations={},
            out_csv=str(Path(tempfile.mkdtemp()) / 'out.csv'))

        self.assertEqual(len(df), 1)
        row = df.iloc[0]
        self.assertIsNone(row['degradation_dim'])
        self.assertIsNone(row['degradation_level'])
...
class ZeroLevelDedupTest(FakePipelineMixin, unittest.TestCase):
    ...
    def test_dv_stage_called_once_for_level_zero(self):
        ...
        with patch('process_voids.metric_context.pvoid.skipprob',
            ...
            run(['fake_log.xes'], combos=self.combos, degradations=self.degradations,
                levels=[0.0, 0.5], out_csv=str(self.tmp_out))
...
class NotImplementedComboTest(unittest.TestCase):
    ...
    def test_skipped_without_computing_anything(self):
        ...
        with patch('lab.run.pm4py.read_xes', return_value='FAKE_LOG'), \
...
class NodeRowsTest(FakePipelineMixin, unittest.TestCase):
    ...
    columns), not re-deriving the metric math.

    The node set comes from lab.run._all_nodes' own tree walk, not the
    classical stage's table (see that function on why), so these cover
    the same nodes either way for a tree the classical stage also spans.
    """
    ...
    def _run(self, **pipeline_kwargs):
        ...
        return run(['fake_log.xes'], combos=self.combos, degradations=self.degradations,
                   levels=[0.5], out_csv=str(self.tmp_out))
...
class TimingRowsTest(FakePipelineMixin, unittest.TestCase):
    """
    run's third return value: one long-form timing row per stage/metric
    actually computed, stamped with the cell's own identity - see
    lab.timing.TimingListener and process_voids.metric_context.CellContext.
    """
...
class DryRunTest(unittest.TestCase):
    """--dry-run prints the resolved Experiment and exits without running
    any real computation - main()'s describe-then-return path must never
    fall through into computing anything."""

    def test_dry_run_with_run_name_prints_and_computes_nothing(self):
        with patch('sys.argv', ['run', '--run', 'smoke', '--dry-run']), \
             patch('lab.run.configure'), \
             patch('process_voids.metric_context.pvoid.skipprob') as mock_skipprob, \
             patch('lab.run.build_id_net') as mock_build_id_net:
            buf = io.StringIO()
    ...
    def test_dry_run_with_ad_hoc_logs_prints_and_computes_nothing(self):
        with patch('sys.argv', ['run', 'fake_log.xes', '--combos', 'inductive_noise20',
                                 '--levels', '0.0', '--dry-run']), \
             patch('lab.run.configure'), \
             patch('process_voids.metric_context.pvoid.skipprob') as mock_skipprob, \
             patch('lab.run.build_id_net') as mock_build_id_net:
    ...
    def test_dry_run_with_no_degradation_flag(self):
        with patch('sys.argv', ['run', 'fake_log.xes', '--combos', 'inductive_noise20',
                                 '--no-degradation', '--dry-run']), \
             patch('lab.run.configure'), \
             patch('process_voids.metric_context.pvoid.skipprob') as mock_skipprob, \
             patch('lab.run.build_id_net') as mock_build_id_net:
            buf = io.StringIO()
    ...
    def test_unknown_combo_name_errors_out(self):
        with patch('sys.argv', ['run', 'fake_log.xes', '--combos', 'not_a_real_combo', '--dry-run']), \
             patch('lab.run.configure'):
